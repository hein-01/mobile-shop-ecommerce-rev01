      ...
      <div className="flex flex-col w-full max-w-[1600px] flex-1">
        ...
        <div className="flex overflow-x-auto gap-4 px-4 pb-8 no-scrollbar snap-x snap-mandatory">
          {products.map((product, index) => {
            // Transform the last card into a "See All" card
            if (index === products.length - 1) {
                return (
                    <div key={index} className="shrink-0 snap-start w-[16vw] min-w-[240px] max-w-[260px]">
                        <div className="group relative flex flex-col h-[480px] rounded-[20px] p-[2px] bg-gradient-to-b from-[#ff6b6b] to-[#4facfe] shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer">
                            <div className="flex flex-col h-full w-full bg-white rounded-[18px] overflow-hidden relative items-center justify-center p-6 text-center hover:bg-gray-50 transition-colors">
                                <div className="w-20 h-20 rounded-full bg-gray-100 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 group-hover:bg-primary group-hover:text-white">
                                    <span className="material-symbols-outlined text-4xl text-gray-600 group-hover:text-white transition-colors">arrow_forward</span>
                                </div>
                                <h3 className="text-2xl font-black text-gray-800 mb-2">See All Products</h3>
                                <p className="text-sm font-bold text-gray-400">Discover our full range of limited time deals</p>
                                <button className="mt-8 px-6 py-3 rounded-full bg-black text-white text-sm font-bold uppercase tracking-wider group-hover:bg-primary transition-colors shadow-md">
                                    View All
                                </button>
                            </div>
                        </div>
